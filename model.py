...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, heads=2, window_size=1, qkv_bias=False, qk_scale=None, dropout=0., causal=True, device=None):
        ...
        if self.trend_aware:
            kernel_size = 3
            self.qk_conv = nn.Conv1d(dim, dim * 2, kernel_size=kernel_size, padding=kernel_size // 2, bias=qkv_bias)
...
class SpatialAttention(nn.Module):
    ...
    def _QK(self, Q, K, sample_k, n_top):
        ...
        index_sample = torch.randint(0, N, (sample_k,), device=Q.device)  
        ...
        Q_K = torch.matmul(Q_reduce, K.transpose(-2, -1))  # (B, H, T, n_top, N)
    ...
    def forward(self, queries, keys, values):
        ...
        if self.num_nodes not in [170, 358]:
            context = context.reshape(B, -1, N, T)
            context = context * self.weight + self.bias + context
            context = context.reshape(B, N, T, -1)
...
class TWIST(nn.Module):
    def __init__(
        ...
        if num_nodes == 170 or num_nodes == 307 or num_nodes == 358  or num_nodes == 883 :
            ...
            time = 48
        elif num_nodes == 2352 or num_nodes == 1918:
            time = 96
